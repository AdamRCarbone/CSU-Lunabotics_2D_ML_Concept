...
export class PhysicsEngine {
  ...
  private timescale: number = 1.0; // Simulation speed multiplier
  ...
  update(deltaTime: number = 16.67) {
    // Apply timescale to deltaTime for speed control
    Engine.update(this.engine, deltaTime * this.timescale);
  }

  // Set simulation timescale (1.0 = normal, 2.0 = 2x speed, 0.5 = half speed)
  setTimescale(timescale: number) {
    this.timescale = Math.max(0.1, Math.min(10.0, timescale)); // Clamp between 0.1x and 10x
    console.log(`[Physics] Timescale set to ${this.timescale}x`);
  }
