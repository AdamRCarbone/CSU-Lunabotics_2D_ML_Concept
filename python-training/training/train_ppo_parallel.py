"""
PPO Training Script with Parallel Environments
Trains faster by running multiple browser environments in parallel
"""
...
def close_browser_tabs_by_url(url_pattern):
    ...
    if system == "Darwin":  # macOS
        ...
        # '''
        # try:
        #     subprocess.run(['osascript', '-e', applescript_safari],
        #                  stdout=subprocess.DEVNULL,
        #                  stderr=subprocess.DEVNULL,
        #                  timeout=5)
        #     print(f"[Cleanup] Closed Safari tabs matching: {url_pattern}")
        # except:
        #     pass  # Safari might not be running

    elif system == "Linux":
        # On Linux, we can try to close Chrome tabs
        # This is trickier and might not work on all systems
...
def main():
    print("=" * 60)
    print("Lunar Rover PPO Training - Parallel Environments")
    print("=" * 60)

    # ============ CONFIGURATION ============

    # Ask user for number of parallel environments
    print("\n[Setup] How many parallel environments do you want?")
    print("[Setup] Recommended: 4 (good balance of speed and stability)")
    print("[Setup] Note: Diminishing returns after ~8 environments")
    while True:
        try:
            num_envs_input = input("[Setup] Enter number of environments (or press Enter for 4): ").strip()
            if num_envs_input == "":
                NUM_ENVS = 4
                break
            NUM_ENVS = int(num_envs_input)
            if NUM_ENVS >= 2:
                break
            else:
                print("[Setup] Please enter a number >= 2")
        except ValueError:
            print("[Setup] Please enter a valid number")

    print(f"\n[Setup] Using {NUM_ENVS} parallel environments (~{NUM_ENVS}x faster training)")

    # Ask user for number of timesteps
    print("\n[Setup] How many timesteps do you want to train for?")
    print("[Setup] Examples: 100000 (100k), 500000 (500k), 1000000 (1M)")
    while True:
        try:
            timesteps_input = input("[Setup] Enter timesteps (or press Enter for 1,000,000): ").strip()
            if timesteps_input == "":
                TOTAL_TIMESTEPS = 1_000_000
                break
            # Handle common suffixes
            timesteps_input_lower = timesteps_input.lower()
            if timesteps_input_lower.endswith('k'):
                TOTAL_TIMESTEPS = int(float(timesteps_input_lower[:-1]) * 1000)
            elif timesteps_input_lower.endswith('m'):
                TOTAL_TIMESTEPS = int(float(timesteps_input_lower[:-1]) * 1_000_000)
            else:
            ...
            if TOTAL_TIMESTEPS > 0:
                ...
                print("[Setup] Please enter a positive number")
        except ValueError:
            print("[Setup] Please enter a valid number (e.g., 1000000, 1m, 500k)")

    print(f"[Setup] Training for {TOTAL_TIMESTEPS:,} timesteps")

    # Ask user for max episode steps
    print("\n[Setup] How many steps per episode (max_episode_steps)?")
    print("[Setup] This controls how long each training episode runs before reset")
    print("[Setup] Examples: 1000 (short), 5000 (medium), 10000 (long - default)")
    while True:
        try:
            max_steps_input = input("[Setup] Enter max episode steps (or press Enter for 5,000): ").strip()
            if max_steps_input == "":
                MAX_EPISODE_STEPS = 5000
                break
            # Handle common suffixes
            max_steps_input_lower = max_steps_input.lower()
            if max_steps_input_lower.endswith('k'):
                MAX_EPISODE_STEPS = int(float(max_steps_input_lower[:-1]) * 1000)
            else:
                MAX_EPISODE_STEPS = int(max_steps_input)
            ...
            print("[Setup] Please enter a valid number (e.g., 1000, 5000, 10k)")

    print(f"[Setup] Max episode steps: {MAX_EPISODE_STEPS:,}")

    TIMESCALE = 10.0  # Simulation speed per environment
    LOG_DIR = "./logs"
    MODEL_DIR = "./models"
    MODEL_NAME = "lunar_rover_ppo"
    AUTO_LOAD_LATEST = True  # Automatically load latest checkpoint if available
    LOAD_MODEL = None  # Override auto-load with specific path (e.g., "./models/lunar_rover_ppo_500000_steps")
    ...
    # Each subprocess will create its own WebSocket server
    ...
    print("[Setup] NOTE: Please manually disable Chrome tab throttling:")
    print("[Setup]   chrome://flags/#expensive-background-timer-throttling")
    print("[Setup]   Set to 'Disabled' and restart Chrome before training")
    print()
    ...
    checkpoint_to_load = LOAD_MODEL
    starting_timestep = 0  # Track where we're continuing from

    if AUTO_LOAD_LATEST and not LOAD_MODEL:
        ...
        if result:
            ...
            # Remove .zip extension for PPO.load
    ...
    # Callbacks
    # Use custom callback that properly continues numbering from loaded checkpoint
